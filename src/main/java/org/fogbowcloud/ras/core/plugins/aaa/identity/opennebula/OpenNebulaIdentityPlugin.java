...
public class OpenNebulaIdentityPlugin implements FederationIdentityPlugin<OpenNebulaToken> {
    ...
    @Override
    public OpenNebulaToken createToken(String tokenValue) throws InvalidTokenException {
        if(tokenValue == null || tokenValue.isEmpty()){
            throw new InvalidTokenException(String.format(Messages.Error.INVALID_TOKEN_VALUE, Messages.Error.EMPTY_OR_NULL_TOKEN));
        }
        ...
        if (split.length != OpenNebulaTokenGeneratorPlugin.FEDERATION_TOKEN_PARAMETER_SIZE) {
            throw new InvalidTokenException(String.format(Messages.Error.INVALID_TOKEN_VALUE, Messages.Error.INVALID_FORMAT_TOKEN));
        }
