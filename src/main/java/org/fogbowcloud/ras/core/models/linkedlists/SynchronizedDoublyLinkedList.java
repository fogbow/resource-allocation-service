...
public class SynchronizedDoublyLinkedList implements ChainedList {
    ...
    public synchronized void addItem(Order order) {
        if (order == null) {
            throw new IllegalArgumentException(Messages.Exception.ATTEMPTING_TO_ADD_A_NULL_REQUEST);
        }
        if (this.head == null) {
    ...
    public synchronized boolean removeItem(Order order) {
        if (order == null) {
            throw new IllegalArgumentException(Messages.Exception.ATTEMPTING_TO_REMOVE_A_NULL_REQUEST);
        }
