...
public class VolumeOrdersController {
    ...
    public ResponseEntity<VolumeInstance> getVolume(@PathVariable String volumeId,
            @RequestHeader(required = false, value = FEDERATION_TOKEN_VALUE_HEADER_KEY) String federationTokenValue)
            throws Exception {
        LOGGER.info(String.format(Messages.Info.RECEIVING_REQUEST_FOR_GET_ORDER, ORDER_CONTROLLER_TYPE, volumeId));
    ...
    public ResponseEntity<Boolean> deleteVolume(@PathVariable String volumeId,
            @RequestHeader(required = false, value = FEDERATION_TOKEN_VALUE_HEADER_KEY) String federationTokenValue)
            throws Exception {
        LOGGER.info(String.format(Messages.Info.RECEIVING_REQUEST_FOR_DELETE_ORDER, ORDER_CONTROLLER_TYPE, volumeId));
