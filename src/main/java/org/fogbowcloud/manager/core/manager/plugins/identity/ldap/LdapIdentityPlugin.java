package org.fogbowcloud.manager.core.manager.plugins.identity.ldap;
...
public class LdapIdentityPlugin implements IdentityPlugin {
	...
	public Token createToken(Map<String, String> userCredentials) throws UnauthorizedException, TokenCreationException {
		...
		try {
			...
			throw new InvalidCredentialsException("Couldn't load account summary from LDAP Server.", e);
		}
	...
	public Token getToken(String accessId) throws UnauthorizedException {
		try {
			...
			return new Token(accessId, new Token.User(uuid, name), expirationDate, new HashMap<String, String>());
	...
	private String encryptPassword(String password) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		return PASSWORD_ENCRYPTED.replaceAll(ENCRYPT_TYPE, this.encryptType).replaceAll(ENCRYPT_PASS, hexString.toString());
