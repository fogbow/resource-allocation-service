...
public class HibernatingProcessor extends StoppableOrderListProcessor implements Runnable {
    private static final Logger LOGGER = Logger.getLogger(HibernatingProcessor.class);

    private String localProviderId;

    public HibernatingProcessor(String localProviderId, String sleepTimeStr) {
        super(Long.valueOf(sleepTimeStr), 
                SharedOrderHolders.getInstance().getHibernatingOrdersList());
    ...
    void processHibernatingOrder(Order order) throws FogbowException {
        ...
        synchronized (order) {
            // Check if the order is still in the HIBERNATING state (for this particular state, this should
            ...
            if (!orderState.equals(OrderState.HIBERNATING)) {
                return;
            }

            // Only compute order can be hibernated.
