...
public class RemoteResumeOrderRequestHandler extends AbstractQueryHandler {
    private static final Logger LOGGER = Logger.getLogger(RemoteResumeOrderRequestHandler.class);

    private static final String REMOTE_RESUME_ORDER = RemoteMethod.REMOTE_RESUME_ORDER.toString();

    public RemoteResumeOrderRequestHandler() {
        super(REMOTE_RESUME_ORDER);
    }
    ...
    public IQ handle(IQ iq) {
        ...
        try {
            ...
            RemoteFacade.getInstance().resumeOrder(senderId, orderId, systemUser, resourceType);
        } catch (Exception e) {
