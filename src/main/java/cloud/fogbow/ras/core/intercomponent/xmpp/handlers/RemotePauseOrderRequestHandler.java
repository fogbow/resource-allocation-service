...
public class RemotePauseOrderRequestHandler extends AbstractQueryHandler {
    private static final Logger LOGGER = Logger.getLogger(RemotePauseOrderRequestHandler.class);

    private static final String REMOTE_PAUSE_ORDER = RemoteMethod.REMOTE_PAUSE_ORDER.toString();

    public RemotePauseOrderRequestHandler() {
        super(REMOTE_PAUSE_ORDER);
    }
    ...
    public IQ handle(IQ iq) {
        ...
        try {
            ...
            RemoteFacade.getInstance().pauseOrder(senderId, orderId, systemUser, resourceType);
        } catch (Exception e) {
