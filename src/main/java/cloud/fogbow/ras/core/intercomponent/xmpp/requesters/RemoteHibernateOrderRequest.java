...
public class RemoteHibernateOrderRequest implements RemoteRequest<Void> {
    private static final Logger LOGGER = Logger.getLogger(RemoteHibernateOrderRequest.class);

    private Order order;

    public RemoteHibernateOrderRequest(Order order) {
        this.order = order;
    }

    @Override
    public Void send() throws Exception {
        IQ iq = RemoteHibernateOrderRequest.marshal(this.order);
    ...
    public static IQ marshal(Order order) {
        ...
        Element queryElement = iq.getElement().addElement(IqElement.QUERY.toString(),
                RemoteMethod.REMOTE_HIBERNATE_ORDER.toString());
