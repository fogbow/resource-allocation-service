...
class NetworkTests:
	...
	def test_post_local_networks(cls):
		extra_data = {}
		response = CommonMethods.post_order(extra_data, GeneralConfigurations.network)
		...
		if CommonMethods.wait_instance_ready(order_id, GeneralConfigurations.network):
			print('Test post local network: Ok. Removing network')
		else:
			print('Test post local network: Failed. Removing network')
		CommonMethods.delete_order(order_id, GeneralConfigurations.network)
	...
	def test_delete_local_network(cls):
		...
		if response.status_code != GeneralConfigurations.created_status:
			...
			return
		order_id = response.text
		get_response = CommonMethods.get_order_by_id(order_id, GeneralConfigurations.network)
