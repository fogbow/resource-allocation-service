...
//        Mockito.doReturn(rule).when(this.plugin).createSecurityRuleRequest(Mockito.eq(securityRule),
//                Mockito.eq(securityGroup));
...
//        Rule rule = buildRule();
    ...
    public void testDoGetSecurityGroupResponse() {
        ...
        GetSecurityGroupResponse securityGroupResponse = buildSecurityGroupResponse(rules);
        PowerMockito.mockStatic(GetSecurityGroupResponse.class);
        PowerMockito.when(GetSecurityGroupResponse.unmarshal(Mockito.eq(xml))).thenReturn(securityGroupResponse);

        // exercise
        this.plugin.doGetSecurityGroupResponse(securityGroup);
        ...
        PowerMockito.verifyStatic(GetSecurityGroupResponse.class, Mockito.times(TestUtils.RUN_ONCE));
        GetSecurityGroupResponse.unmarshal(Mockito.eq(xml));
    }
